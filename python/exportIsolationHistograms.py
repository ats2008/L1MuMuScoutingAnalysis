...
def main():
    ...
    dataStore=addGenMatchedMuonInformation(dataStore)
    ...
    dataStore=addSelectedDimuons(dataStore,addTkIsoMask=False)
    
    dataStore=addPFIsolationVariables(dataStore,'selected_dimu')
    dataStore=addPUPPIIsolationVariables(dataStore,'selected_dimu')

    ARRAY=dataStore[tag]['selected_dimu_mu1']['puppi_rel_iso']
    BINS=np.arange(0.0,7.01,0.05)
    NAME="mu1PUPPIResltiveIsolation"
    DOC="puppi isolation for mu1 in dimuon candidate per event, after selection"
    histStore[NAME]  =getHistograms(ARRAY,bins=BINS,name=NAME,doc=DOC)

    ARRAY=dataStore[tag]['selected_dimu_mu2']['puppi_rel_iso']
    BINS=np.arange(0.0,7.01,0.05)
    NAME="mu2PUPPIResltiveIsolation"
    DOC="puppi isolation for mu2 in dimuon candidate per event, after selection"
    histStore[NAME]  =getHistograms(ARRAY,bins=BINS,name=NAME,doc=DOC)

    ARRAY=dataStore[tag]['selected_dimu_mu1']['pf_rel_iso']
    BINS=np.arange(0.0,7.01,0.05)
    NAME="mu1PFResltiveIsolation"
    DOC="pf isolation for mu1 in dimuon candidate per event, after selection"
    histStore[NAME]  =getHistograms(ARRAY,bins=BINS,name=NAME,doc=DOC)

    ARRAY=dataStore[tag]['selected_dimu_mu2']['pf_rel_iso']
    BINS=np.arange(0.0,7.01,0.05)
    NAME="mu2PFResltiveIsolation"
    DOC="pf isolation for mu2 in dimuon candidate per event, after selection"
    histStore[NAME]  =getHistograms(ARRAY,bins=BINS,name=NAME,doc=DOC)
